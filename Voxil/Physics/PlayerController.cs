...
public class PlayerController
{
    ...
    public void SetPreset(ControllerPreset preset)
    {
        switch (preset)
        {
            case ControllerPreset.Normal: _playerState.Settings = ControllerSettingsPresets.Normal; break;
            case ControllerPreset.DebugLevitate: _playerState.Settings = ControllerSettingsPresets.DebugLevitate; break;
            case ControllerPreset.Spectator: _playerState.Settings = ControllerSettingsPresets.Spectator; break;
        }
    }
    ...
    public void Update(InputManager input, float deltaTime)
    {
        ...
        if (_isFlying)
        {
            ...
            if (input.IsKeyDown(input.Crouch)) targetVel -= System.Numerics.Vector3.UnitY;

            if (targetVel.LengthSquared() > 0.01f)
                bodyReference.Velocity.Linear = System.Numerics.Vector3.Normalize(targetVel) * currentFlySpeed;
            else
