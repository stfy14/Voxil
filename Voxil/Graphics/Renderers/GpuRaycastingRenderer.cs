...
// 1. Если есть свободные слоты - берем их
    ...
    public void UpdateDynamicObjectsAndGrid() { /* (Без изменений) */ var voxelObjects = _worldManager.GetAllVoxelObjects(); int count = voxelObjects.Count; if (count > _tempGpuObjectsArray.Length) { Array.Resize(ref _tempGpuObjectsArray, count + 1024); GL.BindBuffer(BufferTarget.ShaderStorageBuffer, _dynamicObjectsBuffer); GL.BufferData(BufferTarget.ShaderStorageBuffer, _tempGpuObjectsArray.Length * Marshal.SizeOf<GpuDynamicObject>(), IntPtr.Zero, BufferUsageHint.DynamicDraw); } if (count > 0) { for (int i = 0; i < count; i++) { var vo = voxelObjects[i]; Matrix4 model = Matrix4.CreateTranslation(-vo.LocalCenterOfMass) * Matrix4.CreateFromQuaternion(vo.Rotation) * Matrix4.CreateTranslation(vo.Position); var col = MaterialRegistry.GetColor(vo.Material); _tempGpuObjectsArray[i].Model = model; _tempGpuObjectsArray[i].InvModel = Matrix4.Invert(model); _tempGpuObjectsArray[i].Color = new Vector4(col.r, col.g, col.b, 1.0f); _tempGpuObjectsArray[i].BoxMin = new Vector4(vo.LocalBoundsMin, 0); _tempGpuObjectsArray[i].BoxMax = new Vector4(vo.LocalBoundsMax, 0); } GL.BindBuffer(BufferTarget.ShaderStorageBuffer, _dynamicObjectsBuffer); GL.BufferSubData(BufferTarget.ShaderStorageBuffer, IntPtr.Zero, count * Marshal.SizeOf<GpuDynamicObject>(), _tempGpuObjectsArray); } float snap = _gridCellSize; Vector3 playerPos = _worldManager.GetPlayerPosition(); Vector3 snappedCenter = new Vector3((float)Math.Floor(playerPos.X / snap) * snap, (float)Math.Floor(playerPos.Y / snap) * snap, (float)Math.Floor(playerPos.Z / snap) * snap); float halfExtent = (OBJ_GRID_SIZE * _gridCellSize) / 2.0f; _lastGridOrigin = snappedCenter - new Vector3(halfExtent); uint zero = 0; GL.BindBuffer(BufferTarget.AtomicCounterBuffer, _atomicCounterBuffer); GL.BufferSubData(BufferTarget.AtomicCounterBuffer, IntPtr.Zero, sizeof(uint), ref zero); GL.ClearTexImage(_gridHeadTexture, 0, PixelFormat.RedInteger, PixelType.Int, IntPtr.Zero); if (count > 0) { _gridComputeShader.Use(); _gridComputeShader.SetInt("uObjectCount", count); _gridComputeShader.SetVector3("uGridOrigin", _lastGridOrigin); _gridComputeShader.SetFloat("uGridStep", _gridCellSize); _gridComputeShader.SetInt("uGridSize", OBJ_GRID_SIZE); GL.BindImageTexture(1, _gridHeadTexture, 0, true, 0, TextureAccess.ReadWrite, SizedInternalFormat.R32i); GL.BindBufferBase(BufferRangeTarget.ShaderStorageBuffer, 2, _dynamicObjectsBuffer); GL.BindBufferBase(BufferRangeTarget.ShaderStorageBuffer, 3, _linkedListSsbo); GL.BindBufferBase(BufferRangeTarget.AtomicCounterBuffer, 4, _atomicCounterBuffer); GL.DispatchCompute((count + 63) / 64, 1, 1); GL.MemoryBarrier(MemoryBarrierFlags.ShaderImageAccessBarrierBit | MemoryBarrierFlags.AtomicCounterBarrierBit | MemoryBarrierFlags.ShaderStorageBarrierBit); } }
    
    public void Render(Camera cam) { if (_reallocationPending || _activeBankCount == 0) return; _shaderSystem.Use(); var shader = _shaderSystem.RaycastShader; if (shader == null) return; shader.SetVector3("uCamPos", cam.Position); shader.SetMatrix4("uView", cam.GetViewMatrix()); shader.SetMatrix4("uProjection", cam.GetProjectionMatrix()); shader.SetMatrix4("uInvView", Matrix4.Invert(cam.GetViewMatrix())); shader.SetMatrix4("uInvProjection", Matrix4.Invert(cam.GetProjectionMatrix())); float viewRange = _worldManager.GetViewRangeInMeters(); shader.SetFloat("uRenderDistance", viewRange); if (GameSettings.EnableLOD) { float lodMeters = viewRange * GameSettings.LodPercentage; shader.SetFloat("uLodDistance", lodMeters); } else { shader.SetFloat("uLodDistance", 100000.0f); } shader.SetInt("uDisableEffectsOnLOD", GameSettings.DisableEffectsOnLOD ? 1 : 0); shader.SetInt("uMaxRaySteps", (int)(GameSettings.RenderDistance * 8) + 2028); shader.SetVector3("uSunDir", Vector3.Normalize(new Vector3(0.2f, 0.4f, 0.8f))); shader.SetFloat("uTime", _totalTime); shader.SetInt("uShowDebugHeatmap", GameSettings.ShowDebugHeatmap ? 1 : 0); shader.SetInt("uSoftShadowSamples", GameSettings.SoftShadowSamples); Vector3 p = cam.Position; int sz = Constants.ChunkSizeWorld; int cx = (int)Math.Floor(p.X/sz), cy = (int)Math.Floor(p.Y/sz), cz = (int)Math.Floor(p.Z/sz); int r = GameSettings.RenderDistance + 2; shader.SetInt("uBoundMinX", cx-r); shader.SetInt("uBoundMinY", 0); shader.SetInt("uBoundMinZ", cz-r); shader.SetInt("uBoundMaxX", cx+r); shader.SetInt("uBoundMaxY", WorldManager.WorldHeightChunks); shader.SetInt("uBoundMaxZ", cz+r); GL.BindImageTexture(0, _pageTableTexture, 0, true, 0, TextureAccess.ReadOnly, SizedInternalFormat.R32ui); BindAllBuffers(); GL.BindBufferBase(BufferRangeTarget.ShaderStorageBuffer, 5, _maskSsbo); GL.BindImageTexture(1, _gridHeadTexture, 0, true, 0, TextureAccess.ReadOnly, SizedInternalFormat.R32i); GL.BindBufferBase(BufferRangeTarget.ShaderStorageBuffer, 2, _dynamicObjectsBuffer); GL.BindBufferBase(BufferRangeTarget.ShaderStorageBuffer, 3, _linkedListSsbo); shader.SetTexture("uNoiseTexture", _noiseTexture, TextureUnit.Texture0); shader.SetVector3("uGridOrigin", _lastGridOrigin); shader.SetFloat("uGridStep", _gridCellSize); shader.SetInt("uGridSize", OBJ_GRID_SIZE); shader.SetInt("uObjectCount", _worldManager.GetAllVoxelObjects().Count); GL.BindVertexArray(_quadVao); if (GameSettings.BeamOptimization) { if (_beamTexture == 0) { _beamTexture = GL.GenTexture(); GL.BindTexture(TextureTarget.Texture2D, _beamTexture); GL.TexImage2D(TextureTarget.Texture2D, 0, PixelInternalFormat.R32f, _beamWidth, _beamHeight, 0, PixelFormat.Red, PixelType.Float, IntPtr.Zero); GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMinFilter, (int)TextureMinFilter.Nearest); GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMagFilter, (int)TextureMinFilter.Nearest); } GL.BindFramebuffer(FramebufferTarget.Framebuffer, _beamFbo); GL.FramebufferTexture2D(FramebufferTarget.Framebuffer, FramebufferAttachment.ColorAttachment0, TextureTarget.Texture2D, _beamTexture, 0); GL.Viewport(0, 0, _beamWidth, _beamHeight); GL.ClearColor(10000.0f, 0, 0, 1); GL.Clear(ClearBufferMask.ColorBufferBit); shader.SetInt("uIsBeamPass", 1); GL.Disable(EnableCap.DepthTest); GL.DrawArrays(PrimitiveType.TriangleStrip, 0, 4); } GL.BindFramebuffer(FramebufferTarget.Framebuffer, 0); GL.Viewport(0, 0, _windowWidth, _windowHeight); shader.SetInt("uIsBeamPass", 0); shader.SetTexture("uBeamTexture", _beamTexture, TextureUnit.Texture1); GL.DrawArrays(PrimitiveType.TriangleStrip, 0, 4); GL.Enable(EnableCap.DepthTest); GL.BindVertexArray(0); }
