...
// Оригинальный Чебышёв (2 параметра)
    ...
    int bX, int bY, int bZ, float sp, int level)
{
    ...
    // ИСПРАВЛЕНИЕ 1: БЕСКОНЕЧНЫЙ L2
    // Ограничиваем точку сэмплирования границами сетки, чтобы дальние зонды тянули свет в бесконечность
    vec3 clampedSamplePos = samplePos;
    if (level == 2) {
        vec3 gridMax = gridOrigin + vec3(float(uGIProbeX - 1), float(uGIProbeY - 1), float(uGIProbeZ - 1)) * sp;
        clampedSamplePos = clamp(samplePos, gridOrigin, gridMax);
    }
    
    vec3 localPos = (clampedSamplePos - gridOrigin) / sp;
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        vec3  toProbe    = probeWorldPos - clampedSamplePos;
        float probeDist  = length(toProbe);
        
        // (ИСПРАВЛЕНИЕ 2: Убрал if(probeDist > sp * 1.5) continue; Эта строка рвала интерполяцию на швах)
        ...
        if (depthMoments.x < 0.001) continue; 
...
vec3 SampleGIProbes(vec3 worldPos, vec3 normal, vec3 viewDir) {
    ...
    // ИСПРАВЛЕНИЕ 3: Выключаем затухание (fade2) для L2. Теперь L2 - это базовый слой.
    vec3 result = irr2; 
